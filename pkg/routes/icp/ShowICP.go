...
func ShowICP(ctx *fiber.Ctx) error {
	...
	// 	return response.Ctx(ctx).Result(response.Error(400, "GSS243", "O documento do usuário não pode ser vazio."))
	// }
	...
	// 	return response.Ctx(ctx).Result(response.ErrorDefault("GSS244"))
	// }
	...
	// 	return response.Ctx(ctx).Result(response.Error(400, "GSS245", "Não existe um certificado ICP registrado para o usuário "+document))
	// }
