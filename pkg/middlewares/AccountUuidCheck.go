...
func AccountUuidCheck(ctx *fiber.Ctx) error {
	...
	// 	return response.Ctx(ctx).Result(response.ErrorDefault("BLC092"))
	// }

	// if len(accountDatabase.Uuid) == 0 {
	// 	log.Error().Err(err).Msg("Este documento não existe na nossa base de dados.")
	// 	return response.Ctx(ctx).Result(response.Error(400, "BLC094", "Este documento não existe na nossa base de dados."))
	// }
