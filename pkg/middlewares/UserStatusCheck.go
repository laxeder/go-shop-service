...
func UserStatusCkeck(ctx *fiber.Ctx) error {
	...
	// 	return response.Ctx(ctx).Result(response.ErrorDefault("GSS057"))
	// }

	// if userData.Status == user.Disabled {
	// 	log.Error().Msgf("Esta conta está desabilitada por tempo indeterminado. (%v)", document)
	// 	return response.Ctx(ctx).Result(response.Error(404, "GSS058", "Esta conta está desabilitada por tempo indeterminado."))
	// }
