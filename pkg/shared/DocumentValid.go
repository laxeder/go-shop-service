...
func DocumentValid(document string) *response.Result {
	...
	if document == "" {
		...
		return response.Error(400, "GSS010", "O campo do documento não pode ser vazio.")
	}

	if regex.HasLetter.MatchString(document) {
		log.Error().Msgf("O documento não pode conter letras. (%v)", document)
		return response.Error(400, "GSS011", "O documento não pode conter letras.")
	}
	...
	if len(doc) < 11 {
		...
		return response.Error(400, "GSS012", fmt.Sprintf("O documento %v precisa ter no mínimo 11 números.", document))
	}

	if len(doc) > 14 {
		log.Error().Msgf("O documento não pode ter mais que  14 números. (%v)", document)
		return response.Error(400, "GSS013", "O documento não pode ter mais que 14 números.")
	}

	if !str.DocumentValid(doc) {
		log.Error().Msgf("Documento inválido! Favor digitar um documento válido. (%v)", document)
		return response.Error(400, "GSS014", "Documento inválido! Favor digitar um documento válido.")
	}
