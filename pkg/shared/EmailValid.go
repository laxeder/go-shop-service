...
func EmailValid(email string) *response.Result {
	...
	if email == "" {
		...
		return response.Error(400, "BLC015", "O campo do email não pode ser vazio.")
	}

	if len(email) <= 6 {
		log.Error().Msgf("O email não poder ser menor que 6 caracteres. (%v)", email)
		return response.Error(400, "BLC016", "O email não poder ser menor que 6 caracteres.")
	}

	if !regex.IsEmail.MatchString(email) {
		log.Error().Msgf("Email inválido, tente inserir outro email (%v)", email)
		return response.Error(400, "BLC017", "Email inválido, tente inserir outro email.")
	}
