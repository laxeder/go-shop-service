...
func UserCheck(uid string) (userData *user.User, result *response.Result) {
	...
	if err != nil {
		...
		result = response.ErrorDefault("GSS258")
		return
	}
	...
	if len(userData.Document) == 0 {
		...
		result = response.Error(400, "GSS259", "Este documento não existe na nossa base de dados.")
		return
	}
	...
	if userData.Status == user.Disabled {
		...
		result = response.Error(400, "GSS260", "Esta conta está desabilitada por tempo indeterminado.")
		return
	}
