...
func PasswordValid(password, document string) *response.Result {
	...
	if password == "" {
		...
		return response.Error(400, "GSS023", "O campo da senha não pode ser vazio.")
	}

	if len(password) < 8 {
		log.Error().Msgf("O campo da senha precisa ter no mínimo 8 caracteres. (%v)", document)
		return response.Error(400, "GSS024", "O campo da senha precisa ter no mínimo 8 caracteres.")
	}
	...
	if !regex.HasUpppercase.MatchString(password) {
		...
		return response.Error(400, "GSS025", "A senha precisa ter pelo menos um caracter maiúsculo.")
	}

	if !regex.HasLowercase.MatchString(password) {
		log.Error().Msgf("A senha precisa ter pelo menos um caracter minúsculo. (%v)", document)
		return response.Error(400, "GSS026", "A senha precisa ter pelo menos um caracter minúsculo.")
	}

	if !regex.HasNumber.MatchString(password) {
		log.Error().Msgf("A senha precisa ter pelo menos um número. (%v)", document)
		return response.Error(400, "GSS027", "A senha precisa ter pelo menos um número.")
	}
	...
	if !regex.HasCharSpecials.MatchString(password) {
		...
		return response.Error(400, "GSS028", "A senha precisa ter pelo menos um caractere especial.")
	}
