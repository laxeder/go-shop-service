...
func NameValid(name string) *response.Result {
	...
	if name == "" {
		...
		return response.Error(400, "BLC004", "O campo do nome não pode ser vazio.")
	}

	if len(name) <= 2 {
		log.Error().Msgf("O nome não poder ser menor que 2 caracteres. (%v)", name)
		return response.Error(400, "BLC005", "O nome não poder ser menor que 2 caracteres.")
	}

	if regex.HasNumber.MatchString(name) {
		log.Error().Msgf("O nome não pode conter número. (%v)", name)
		return response.Error(400, "BLC006", "O nome não pode conter número.")
	}
